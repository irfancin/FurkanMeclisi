...
export default function RaporlarPage() {
  ...
  const kayitGir = async (kullanici_id: string, cuz_no: number) => {
    ...
    setKayitGiriliyor(prev => new Set(prev).add(key))
    try {
      ...
      setKayitGiriliyor(prev => { const s = new Set(prev); s.delete(key); return s })
    }
  }
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              {okumayan.length === 0 ? (
                ...
                <ul className="divide-y divide-slate-100">
                  {okumayan.map(u => {
                    const key = `${u.id}_${u.cuz_no}`
                    const yukleniyor = kayitGiriliyor.has(key)
                    return (
                      <li key={key} className="flex items-center justify-between px-4 py-3">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => u.cuz_no !== null && kayitGir(u.id, u.cuz_no)}
                            disabled={yukleniyor || u.cuz_no === null}
                            className="flex items-center gap-1 bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-700 text-xs font-medium px-3 py-1.5 rounded-full transition-colors disabled:opacity-50"
                          >
                            {yukleniyor ? '⏳' : '✍️'} Kaydet
                          </button>
